...
`dwolla-swagger-python` is available on [PyPi](https://pypi.python.org/pypi/dwolla-swagger-python), and therefore can be installed automagically via [pip](https://pip.pypa.io/en/latest/installing.html).

*To install via pip:*

```
pip install dwolla-swagger-python
```
...
`dwolla-swagger-python` makes it easy for developers to hit the ground running with our API. Before attempting the following, you should ideally create [an application key and secret](https://www.dwolla.com/applications).
...
To create a customer, we will first need to instantiate a `CreateCustomer` model and populate its fields. Afterwards, we can call `create()` on `customers_api`. 
